  ...
  <div class="container">
      <div class="row">
        <div class="col-lg-12 col-md-12">
          ...
          $con = new DB();
          $res = $con->selectWithIdVar("comment","id",$num);

          // $dt = "SELECT * FROM `blogs` WHERE id=$num";
          // $result = mysqli_query($db,$dt) or die( mysqli_error($db));

          	while ($row = mysqli_fetch_array($res)) {
            		echo "<div id='rows' style='text-align: center;background-color: #E6E6FA;'><h6><a href='../blogdetail.php?id={$row['id']}'></a></h6>
            		Текст comment: {$row['body']}<div id='user'>User : {$row['user']}</a></div>
            		<br></div><hr>";
...
<!-- form for the text messege -->
          ...
          <form method="post" action="../../core/admin/editcomment.php">
       ...
       <div class="mb-3">
         ...
         <div class="input-group">
           <input name="name" type="text" class="form-control" id="title" required value="<?php echo $row['user']; ?>">
         </div>
       </div>
